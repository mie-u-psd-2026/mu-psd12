...
> テスト先行作成済み（red状態）: 下記チェックボックスのうち★印はtests/backend・tests/frontend・tests/e2eに対応する失敗テストを追加済み。実装完了後にテストがPASSすることを確認しチェックを付けること。
...
- [ ] APIリクエスト・レスポンスボディの統一（仕様書8.2に合わせる）★
  ...
  - [ ] FE: 上記API形式に呼び出し・レスポンス処理を合わせ、対応するフロントエンドテストを確認する★
  - [ ] BE: `GET /state`の値のJSON二重エンコードを解消する（既存テストも二重エンコードを期待しているため、期待値の修正が必要）

> 9/9: BEの上記7項目を実装し、バックエンド既存テスト25件がPASS。FE対応と状態値の二重エンコード解消は未完了。
